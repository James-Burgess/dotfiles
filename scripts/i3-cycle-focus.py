...
class FocusWatcher:
    ...
    async def on_window_focus(self, i3conn, event):
        logging.info(f"got window focus event {event.container.id}")
        ...
        if window is not None:
            proc = await asyncio.create_subprocess_shell(
                    "i3-msg -t get_tree | jq --raw-output 'recurse(.nodes[]; .nodes != null) | select(.nodes[].focused).layout'",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE)

            workspace = window.workspace()
            if workspace is not None and len(workspace.leaves()) > 2:

                layout, _ = await proc.communicate()
                layout = str(layout, 'utf-8')
                if layout in ['splith\n', 'splitv\n']:
                    for node in workspace.leaves():
                        if node.id in self.window_list:
                            logging.info('not handling this split window')
                            return
